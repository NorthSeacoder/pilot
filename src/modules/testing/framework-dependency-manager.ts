...
export class FrameworkDependencyManager {
  ...
  static getReactDependencies(reactVersion: string, isTypeScript: boolean): DependencySpec[] {
    ...
    // 基础 React 测试依赖
    dependencies.push(
      { name: '@testing-library/react', version: this.getReactTestingLibraryVersion(reactVersion), dev: true },
      { name: '@testing-library/jest-dom', version: this.getJestDomVersion(reactVersion), dev: true },
    ...
    if (isTypeScript) {
      dependencies.push(
        { name: '@types/react', version: this.getReactTypesVersion(reactVersion), dev: true, optional: true },
        { name: '@types/react-dom', version: this.getReactTypesVersion(reactVersion), dev: true, optional: true }
      )
    }
  ...
  static getVue2Dependencies(vueVersion: string, isTypeScript: boolean): DependencySpec[] {
    ...
    dependencies.push(
      ...
      { name: '@testing-library/user-event', version: '^14.0.0', dev: true },
      { name: 'vue-template-compiler', version: this.getVueTemplateCompilerVersion(vueVersion), dev: true }
    )
  ...
  static getVue3Dependencies(_vueVersion: string, isTypeScript: boolean): DependencySpec[] {
    ...
    if (isTypeScript) {
      dependencies.push(
        { name: '@vue/tsconfig', version: '^0.4.0', dev: true, optional: true }
      )
    }
  ...
  static validateDependencyConfiguration(
    ...
    for (const dep of dependencies) {
      if (dep.version && !this.isDependencyCompatible(dep.name, dep.version, techStack, frameworkVersion)) {
