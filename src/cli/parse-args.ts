    ...
    try {
      ...
      program
        ...
        .action((module: string, options: any) => {
          ...
          // 验证架构参数
          if (options.arch && !['single', 'pnpm-workspace', 'yarn-workspace'].includes(options.arch)) {
          ...
          // 验证选项组合
          const exclusiveOptions = [options.config, options.rules, options.deps, options.setup].filter(Boolean)
