...
export async function detectProject(options: ModuleOptions): Promise<ProjectDetection> {
  ...
  // 2. 从当前 package.json 的上级目录开始，继续向上找下一个
  const parentPackageJsonPath = await findUp('package.json', {
    cwd: path.dirname(currentDir), // 这里就是从上级开始找
  })
  const rootDir = parentPackageJsonPath ? path.dirname(parentPackageJsonPath) : currentDir
  // 读取 package.json
  ...
  const [
    ...
    // 检测技术栈
    options.stack
      ? Promise.resolve(options.stack)
      : detectFramework(packageJson, { currentDir, rootDir }),
    // 检测架构
